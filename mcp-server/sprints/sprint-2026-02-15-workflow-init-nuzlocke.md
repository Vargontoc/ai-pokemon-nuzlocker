...
### Objetivos
- [x] Implementar workflow `init_nuzlocke` (Setup one-shot)
    - [x] `Workflows/Setup/InitNuzlockeWorkflow.cs`: Workflow de inicialización de partida Nuzlocke
    ...
    - [x] Refactorizar sistema de persistencia para nueva estructura
        - [x] `INuzlockeFileManager` (nuevo): interfaz para operaciones de fichero por nuzlocke
        ...
        - [x] `NuzlockeFileManager`: implementación con ficheros JSON separados
        - [x] Adaptar `IStateManager` / `StateManager` para delegar en `INuzlockeFileManager`
        - [x] Mantener compatibilidad: endpoints de sesión existentes siguen funcionando
    - [x] Registrar `InitNuzlockeWorkflow` + `INuzlockeFileManager` en DI (`Program.cs`)
    - [x] Tests: `InitNuzlockeWorkflowTests.cs`
        - [x] Validación: generation requerido, gen != 1 rechazada, base_path requerido
        - [x] Estructura de carpetas creada correctamente
        - [x] .nuzlocke contiene metadata correcta (id, gen, type, fecha)
        - [x] game_state.json inicializado con Generation y LockeType
        - [x] GenerateAdvice: prompt contiene generación y tipo de locke
    - [x] Tests: `NuzlockeFileManagerTests.cs`
        - [x] CreateNuzlocke crea estructura completa
        - [x] ListNuzlockes descubre nuzlockes existentes
        - [x] Load/Save GameState + BattleState
        - [x] SaveBattleRecord genera fichero individual en results/
        - [x] DeleteBattleState limpia temporal
    - [x] Test manual: `curl POST /nuzlocke/workflow` con `init_nuzlocke`
    - [x] En {workspace}/app estaria el cliente web que consumiria estos workflows. Agregar o crear si no existiera un fichero llamado workflow.md que describa el workflow creado, los parametros que necesita y la respuesta que espera. Sin grandes añadidos es simplemente para que el equipo de front pueda implementarlo.

### Aprobación Sprint review
- [x] 127/127 tests passing (102 existentes + 25 nuevos)

### Riesgos
- [x] Refactorizar persistencia puede romper tests existentes que usan `InMemoryStateManager` — mitigado: StateManager acepta INuzlockeFileManager como opcional, sin cambios en tests existentes
- [x] El front DEBE configurar `base_path` antes de poder operar — si no lo hace, ningún workflow funciona
- [x] Concurrencia en ficheros — reutilizado patrón `SemaphoreSlim` por nuzlockeId en NuzlockeFileManager + ConcurrentDictionary para locks y path cache
