...
- **Nuzlocke management**
  ...
  - Entregables implementados:
    - [x] `INuzlockeSessionManager` + `NuzlockeSessionManager` implementados y registrados en DI.
    - [x] Persistencia por sesión: archivo oculto `.nuzlocke` en el directorio de la sesión y registro central `nuzlocke_registry.json`.
    - [x] Control de concurrencia por sesión usando `SemaphoreSlim` en `NuzlockeSessionManager`.
    - [x] Endpoints HTTP añadidos en `Program.cs`:
    ...
    - [x] `StateManager` refactorizado para soportar overloads con `sessionId` y delegar a `INuzlockeSessionManager` cuando corresponde.
    - [x] Kernel/tools/plugin/agent actualizados para ser `sessionId`-aware.
    - [x] Tests unitarios añadidos: `NuzlockeSessionManagerTests.cs`.
  ...
  - Estado actual y criterios de aceptación:
    ...
    - Tests unitarios para el manager — COMPLETADO.
...
- **LLM: streaming / retries**
  - [x] Streaming implementado: `NuzlockeAgent.StreamAdviceAsync` con `IAsyncEnumerable<string>`, endpoint SSE `POST /nuzlocke/advice/stream`, 2 tests unitarios (`StreamingTests.cs`).
  - [-] Retries/backoff: no implementado en este sprint. Pendiente para próximo sprint.
...
- [x] Tests unitarios e integración relevantes pasan en verde (49/49 passed)
- [-] `NuzlockeAgent` responde de forma estable con y sin tools (no validado en integración real)

### Riesgos
- Cambios en formato de respuesta pueden romper consumidores si no se coordinan

### Fallos
- Ningún fallo crítico. Todos los tests pasan.

### Sugerencias para el próximo Sprint
- **Context memory**: Diseñar e implementar `game_state` persistente y `battle_context` temporal por sesión
