...
- [x] Implementar workflow `capture_pokemon`
    ...
    - [x] Tests manuales:
        - [x] `curl POST /nuzlocke/workflow` con `capture_pokemon` — captura exitosa (equipo vacío, va al team)
        - [x] `curl POST /nuzlocke/workflow` con `capture_pokemon` — captura con equipo lleno (va al PC)
        - [x] `curl POST /nuzlocke/workflow` con `capture_pokemon` — location duplicada (error regla nuzlocke)
