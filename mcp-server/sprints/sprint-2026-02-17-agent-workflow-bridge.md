...
- [x] Tests passing (214: 202 existentes + 8 bridge + 3 async agent + 1 nuzlocke validation)
...
- [x] Si no encuentra el nuzlocke_id de la session se cancela la operación lanzando un error que se enviara tambien por el websocket. → Validación en `AdviceBackgroundDispatcher.ExecuteAgentAdviceAsync` con `INuzlockeFileManager.GetNuzlockePathAsync`
...
- [ ] **Workflow**: `use_item`: El usuario indica que ha usado un objeto, se resta en inventario se elimina si es 0 (salvo objetos clave o no consumibles). Y si conoce el efecto ejecuta dicho efecto. Por ejemplo si el efecto de la pocion es "Curar 20 ps" hace dicha operación sobre el pokemon que menciona el usuario si fuera asi.
